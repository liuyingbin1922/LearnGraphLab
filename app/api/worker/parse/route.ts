...
function assertAuth(req: Request) {
  const expected = process.env.TASKS_AUTH_BEARER || '';
...
- slug 必须是英文小写+短横线
- 图片信息不完整也要尽力输出，并降低 confidence
- workedExamples：优先基于图片题目；若无法，则给最小例题
...
export async function POST(req: Request) {
  try {
    ...
    const gsUri = String(body?.gsUri || '').trim();
    const mimeType = String(body?.mimeType || 'image/png').trim();
    const hint = String(body?.hint || '').trim();

    if (!gsUri.startsWith('gs://')) {
      return NextResponse.json({ error: 'Invalid gsUri' }, { status: 400 });
    }
    ...
    await db.collection('mistakes').doc(mistakeId).set({
      id: mistakeId,
      userId,
      createdAt: now,
      updatedAt: now,
